...
E = ElementMaker(splns, nsmap) #Note: see now your E is operating in the correct namespace
...
def read_spl(spl_file): 
    ...
    for substanceElement in root.xpath(".//identifiedSubstance/identifiedSubstance", nsmap):
        molfileText = substance.xpath("./moiety/subjectOf/characteristic/value[@mediaType='application/x-mdl-molfile']", nsmap)[0].text
    ...
    for reactionElement in root.xpath(".//processStep/component/processStep", nsmap):
        ...
        for reactantElement in reationElement.xpath("./interactor[@typeCode = 'CSM']/identifiedSubstance/identifiedSubstance", nsmap):
            reaction.AddReactantTemplate(elementChemMap[reactantElement])
        for productElement in reationElement.xpath("./interactor[@typeCode = 'PRD']/identifiedSubstance/identifiedSubstance", nsmap):
            reaction.AddProductTemplate(elementChemMap[productElement])
...
def createReactionElement(reactionObject):                             
    ...
    for i in range(reactionObject.GetNumReactantTemplates()):
      substanceObject = reaction.GetReactantTemplate(i)
      reactionElement.append(
        E("interactor", 
          E("priorityNumber, value=str(priorityNumber++)),
          E("functionCode",code=code,codeSystem="1.3.6.1.4.1.32366.1.1"), "reactant"),
          createSubstanceElement(substanceObject)))
    for i in range(reactionObject.GetNumProductTemplates()):
      substanceObject = reaction.GetProductTemplate(i) #XXX: I don't think we need to subtract the number of reactants here, else we go in with the priorityNumber directly
      reactionElement.append(
        E("interactor", 
          E("priorityNumber, value=str(priorityNumber++)),
          E("functionCode",code=code,codeSystem="1.3.6.1.4.1.32366.1.1"), "product"),
          createSubstanceElement(substanceObject)))                         
